...
/*   Updated: 2026/03/12 12:49:40 by nacuna-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_dda(t_game *game, t_dda *dda)
{
	...
	init_dda_step(game, dda->ray_dir_x, dda->ray_dir_y, dda);
}
...
static void	draw_column(t_game *game, int x, t_dda *dda, int win_h)
{
	...
	if (limits[1] >= win_h)
		limits[1] = win_h - 1;
	draw_texture(game, x, dda, limits, perp_dist);
}
...
void	render_walls(t_game *game)
{
	...
	while (x < game->win_w)
	{
		init_ray(game, x, &dda.ray_dir_x, &dda.ray_dir_y);
		init_dda(game, &dda);
		run_dda(game, &dda);
		draw_column(game, x, &dda, game->win_h);
		x++;
	}
}
